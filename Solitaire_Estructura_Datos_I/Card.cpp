...
void Card::init(int suit, int value) {
    ...
    flip_state = CARD_FACE_DOWN; //Esto para animacion "aun error" ->Revisar oscarrrrrr!!!
    animation_state = -1;
    animation = NULL;
}

void Card::init_animation(int animation_state, Position p, float frames) { //Esto para el movimiento
...
void Card::init_animation(int animation_state, float frames) { //Rotando la animacion
    this->animation_state = animation_state;
    animation = new Flip(frames);
    
    //No es necesario estar cambiando el destino o las coordenadas porque ya está allí y se queda.!!
}
...
void Card::next() {
    if (animation != NULL) {
        ...
        if (animation->get_hecho()) {
            if (animation_state == STATE_ANIMATION_MOVE_AND_FLIP_CARD) {
                init_animation(STATE_ANIMATION_FLIP_CARD, animation->get_cuadros());
            } else {
                delete animation;
                animation = NULL;
            }
...
void Card::finish() {
    if (animation != NULL) {
        ...
        if (animation_state == STATE_ANIMATION_MOVE_AND_FLIP_CARD) {
            init_animation(STATE_ANIMATION_FLIP_CARD, animation->get_cuadros());
            animation->finish(); //Esta parte hace que la animacion se quede en la proxima vez que se llame.!!
...
// Estado de las cartas y se obtiene el valor, color y paquete que pertenece.
...
int Card::get_flip_state() {
    return flip_state;
}
int Card::get_animation_state() {
    return animation_state;
}

void Card::set_scale(sf::Vector2f scale) {
    s.setScale(scale);
}
sf::Vector2f Card::get_scale() {
    return s.getScale();
}
...
sf::Vector2f Card::get_top_left() {
    return sf::Vector2f(get_left(), get_top());
}
sf::Vector2f Card::get_top_right() {
    return sf::Vector2f(get_right(), get_top()); }
sf::Vector2f Card::get_top_center() { return sf::Vector2f(get_center_x(), get_top());
}
sf::Vector2f Card::get_bottom_left() {
    return sf::Vector2f(get_left(), get_bottom());
}
sf::Vector2f Card::get_bottom_right() {
    return sf::Vector2f(get_right(), get_bottom());
}
sf::Vector2f Card::get_center() {
    return sf::Vector2f(get_center_x(), get_center_y());
}
float Card::get_center_x() {
    return s.getPosition().x;
}
float Card::get_center_y() {
    return s.getPosition().y;
}
float Card::get_left() {
    return get_center_x() - CARD_SPRITE_WIDTH / 2;
}
float Card::get_right() {
    return get_center_x() + CARD_SPRITE_WIDTH / 2;
}
float Card::get_top() {
    return get_center_y() - CARD_SPRITE_HEIGHT / 2;
}
float Card::get_bottom() {
    return get_center_y() + CARD_SPRITE_HEIGHT / 2;
}
float Card::get_width() {
    return s.getTextureRect().width;
}
float Card::get_height() {
